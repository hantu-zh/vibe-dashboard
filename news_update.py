...
def fetch_netease():
    ...
    try:
        ...
        if raw.startswith('artiList(') and raw.endswith(')'):

            raw = raw[9:-1]

            data = json.loads(raw)

            articles = data.get('BA8EE5GMwangning', [])

            for a in articles:

                title = a.get('title', '')

                intro = a.get('digest', a.get('intro', a.get('title', '')))

                raw_time = a.get('ptime', '')
                ...
                # 网易文章的url字段有时为空，用docid构造

                article_url = a.get('url', a.get('link', ''))
                ...
                if not article_url:
                    ...
                    if docid:
                        ...
                        article_url = f'https://m.163.com/money/article/{docid}.html'

                items.append({

                    'id': '163' + a.get('docid', str(abs(hash(title)) % 10**10)),
            ...
            print(f'  网易财经: {len(articles)} items')
