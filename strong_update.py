...
# vibe-dashboard repo 的 origin remote token
...
def update_html(today, stocks):
    ...
    # 替换 updated 字段
    _data_match = re.search(r'(_data\s*=\s*\{[^;]+\})', html, re.DOTALL)
    if not _data_match:
        print("[strong_update] ERROR: _data block not found!")
        return False
    
    # 提取 _data 块的完整内容
    data_start = _data_match.start()
    # 找到 _data = { 的位置
    data_begin = html.find("_data = {", data_start)
    if data_begin == -1:
        data_begin = html.find('var _data = {', data_start)
    
    # 找到 _data 块的结束位置（匹配花括号）
    depth = 0
    data_end = data_begin
    started = False
    for i in range(data_begin, len(html)):
        c = html[i]
        if c == '{':
            depth += 1
            started = True
    ...
    old_data_block = html[data_begin:data_end]
    
    # 保留已有的 yimeng 数据，避免覆盖
    yimeng_data = "[]"
    ym = re.search(r'"yimeng"\s*:\s*(\[[^\]]+\])', old_data_block, re.DOTALL)
    if ym:
    ...
    new_html = html[:data_begin] + new_data_block + html[data_end:]
